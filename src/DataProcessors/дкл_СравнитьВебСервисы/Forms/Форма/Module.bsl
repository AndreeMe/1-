...
Процедура Сравнить(Команда)
	ИмяWSСсылки = "ExternalIntegration_21";
	МестоположениеWSDL = "http://37.29.67.47:8082/lift/integration/services/externalIntegration.wsdl";
	СравнитьНаСервере(ИмяWSСсылки, МестоположениеWSDL);
КонецПроцедуры

&НаСервере                                                 
Процедура СравнитьНаСервере(ИмяWSСсылки, МестоположениеWSDL)
...
Процедура СравнитьСервисы(Сервис_1, Сервис_2, ЕстьОтличия)   
	...
	Для Каждого ТочкаПодключения_1 Из Сервис_1.ТочкиПодключения Цикл
		СравнитьТочкиПодключения(ТочкаПодключения_1, Сервис_2.ТочкиПодключения[Сч], ЕстьОтличия);
		Сч = Сч + 1;
	КонецЦикла;
...
Процедура СравнитьТочкиПодключения(ТочкаПодключения_1, ТочкаПодключения_2, ЕстьОтличия)
	...
	Если НЕ ТочкаПодключения_1.Местоположение = ТочкаПодключения_2.Местоположение Тогда
		Текст = "Отличаются имена точек подключения %1 и %2";
		Текст = СтрШаблон(Текст, ТочкаПодключения_1.Местоположение, ТочкаПодключения_2.Местоположение);
		ОбщегоНазначения.СообщитьПользователю(Текст);
		ЕстьОтличия = Истина;
	КонецЕсли;	
...
Процедура СравнитьОперации(Операция_1, Операция_2, ЕстьОтличия)
	...
	Если НЕ Операция_1.Имя = Операция_1.Имя Тогда
