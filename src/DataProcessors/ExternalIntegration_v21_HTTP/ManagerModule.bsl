...
Функция ПолучитьСтатусСклада(НастройкаСклада) Экспорт
	...
	Попытка
		...
		Адрес = дкл_ОбщегоНазначенияПовтИсп.АдресАСХ(НастройкаСклада); 
		Ответ = дкл_Коннектор.GetJson(Адрес + "/system/health/");         
		Ответ = дкл_ОбщегоНазначенияСервер.ПолучитьСтруктуруИзСоответствия(Ответ);
...
Функция ПолучитьСписокПолокСкладаЛифта(НастройкаСклада) Экспорт 
	...
	Адрес = дкл_ОбщегоНазначенияПовтИсп.АдресАСХ(НастройкаСклада); 
	Ответ = дкл_Коннектор.GetJson(Адрес + "/trays");         
	Возврат дкл_ОбщегоНазначенияСервер.ПолучитьСтруктуруИзСоответствия(Ответ);
